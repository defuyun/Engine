...
void drawLine(int id) {
	...
	gl_Position = gl_in[id].gl_Position + vec4(length * scale * gs_in[id].normal, 0.0f);
	EmitVertex();
...
void drawLine2(vec4 midpoint, int index) {
	...
	vec4 start = index > 0 ? length * scale * index * vec4(gs_in[index - 1].normal, 0.0f) + midpoint : midpoint;
	vec4 end = length * scale * (index+1) * vec4(gs_in[index].normal, 0.0f) + start;
